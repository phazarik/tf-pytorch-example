#!/usr/bin/env python3
# -------------------------------------------------------------------------
# This is a simple TensorFlow-based DNN evaluation script that
# - Reads into text files containing physics-processes.
# - Evaluates a .keras model for two classes of physics-processes: WZ and ZZ
...
def main():
    ...
    ## Input files (I am using the same files as the trainer. In principle, you should use something else)
    file_wz = "input_datasets/input_WZ.txt"
    file_zz = "input_datasets/input_ZZ.txt"

    ## Read datasets and give labels (labels are only given for testing purposes)
    df_wz = read_txt_into_df(file_wz, truth=0)  # signal
    df_zz = read_txt_into_df(file_zz, truth=1)  # background
    ...
    ## Input feature names (keep same as trainer; order is important)
    ...
    ## Prepare numpy arrays and apply min-max scaling from the text files
    ...
    ## Load model
    print(f"\n{YELLOW}Loading model: {modelfile}{RESET}")
    ...
    ## Prepare histograms and ROC plots (similar to trainer)
    ...
    if density_:
        ...
        ## No train-normalisation step here; show eval with errors
    ...
    ## ROC plot: signal efficiency vs background rejection (trainer used tpr vs 1-fpr)
...
# -----------------------------------------------------------------------
# Utility functions (kept similar to trainer)
# -----------------------------------------------------------------------
...
def ApplyMinMax(X, min_filename, max_filename):
    if not os.path.exists(min_filename) or not os.path.exists(max_filename):
        raise FileNotFoundError("Min/Max scaling files not found.")
